...
moe_nlp_root = current_dir.parent


@dataclass
class Config:
    # --- Project Metadata ---
    signature: str = 'v7_matmoe_v2_scale_factor'
    version: str = "7.2.2"
    description: str = "MatMoE v2 Scale Factor + Grad Accum + Dynamic Top-K + Stop Gradient KL + Encoder MSE"
    seed: int = 42

    # --- Directory Allocation ---
    artifact_root: Path = moe_nlp_root / 'artifacts'
    ...
    train_max_length_input: int = 128
    train_max_length_output: int = 128
    max_length_inference: int = 128
    lang_pair: str = 'en<->vi'
    ...
    # 🌟 HARDWARE SCALE UP
    # =======================================================
    num_accelerator: int = 4  # 🚀 Upgraded to 8 GPUs
    batch_size: int = 2 ** 10
    ...
    elastic_mlp_dims: List[int] = field(default_factory=lambda: [1024, 512, 256])
    elastic_mlp_probs: List[float] = field(default_factory=lambda: [1/3, 1/3, 1/3])
    use_r_drop: bool = True
    stop_gradient_kl: bool = False
    
    # --- Dynamic Top-K ---
    elastic_top_ks: List[int] = field(default_factory=lambda: [2])
    elastic_top_k_probs: List[float] = field(default_factory=lambda: [1.0])
    k_warmup_steps: int = 0
    k_transition_steps: int = 0
